...
fn read_v<'s>(
    ...
    match get_attribute(c_element.attributes(), QName(b"t"))? {
        ...
        Some(b"str") => {
            ...
            //
            // Per the spec, t="str" always means the cached formula result is a
            // string, so it must stay a string. Parsing it as a float first (the
            // upstream default) silently loses precision for any all-digit string
            // longer than ~15-17 characters (e.g. TikTok/Shopee order IDs), since
            // f64 can't represent them exactly - producing wrong, colliding IDs.
            Ok(DataRef::String(v))
        }
        Some(b"n") => {
            // n - number
